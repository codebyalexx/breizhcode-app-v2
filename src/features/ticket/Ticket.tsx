...
import { CalendarX, Flag, UserRoundCheck } from "lucide-react";
...
export const Ticket = ({ ticket }: { ticket: any }) => {
  ...
  return (
    <div className={"mb-2 rounded-lg border border-accent mx-2.5 bg-white"}>
      <header
        className={
      ...
      >
        ...
        <h2 className={"p-2 text-right font-bold text-lg"}>
          {ticket.type} <span className={"block -mt-1"}>{ticket.subtype}</span>
        </h2>
      </header>
      <div className={"p-2"}>
        <div className={"flex w-full justify-end mb-3"}>
          <span
            className={
              "bg-success-light text-success font-semibold rounded-full px-1 py-1 text-xs flex items-center"
            }
          >
            <span
              className={
                "inline-block h-2.5 w-2.5 bg-success rounded-full mr-1.5"
              }
            ></span>
            en cours
          </span>
        ...
        <div className={"mb-3 flex justify-between w-full"}>
          <div>
            ...
            <span className={"flex font-bold items-center"}>
              ...
              {padWithLeadingZeros(date.getMonth() + 1)}/{date.getFullYear()} à{" "}
        ...
        <Link
          href={"/tickets/display"}
          className={
            "block bg-accent w-full text-white text-center rounded p-2.5"
          }
        >
