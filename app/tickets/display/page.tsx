...
export default async function DisplayTicket() {
    ...
    return (<div className={'flex flex-col w-full h-full'}>
        <header className={'w-full mb-5 relative'}>
            ...
            <img src="/qrcodebanner.png" alt="qr code banner sample" className={'w-full'}/>
        </header>
        <div className={'w-full h-full pr-1'}>
            <div className={'flex flex-col items-center justify-between px-2 pb-10 w-full h-full bg-[#a8ccec] rounded-t-[80px]'}>
                <div className={'py-14 flex items-center justify-center'}>
                    <Spinner />
                </div>
                <div className={'px-0 w-full flex flex-col items-center'}>
                    <div className={'w-full mb-2 rounded-lg border border-accent bg-white'}>
                        ...
                        <div className={'p-2'}>
                            ...
                            <span className={'text-lg font-bold flex items-center mb-1'}>
                                <Flag size={20} className={'text-accent mr-2'} /> {ticket.from}
                            </span>
                                <span className={'text-lg font-bold flex items-center mb-3'}>
                                <Flag size={20} className={'text-accent mr-2'} /> {ticket.to}
                            </span>
                            </div>
                            <div className={'flex justify-between w-full'}>
                                <div>
                                    ...
                                    <CalendarCheck size={20} className={'text-success mr-1.5'} /> {createdAt.getDate()}/{createdAt.getMonth()+1}/{createdAt.getFullYear()} à {createdAt.getHours()}:{createdAt.getMinutes()}
                                </span>
                                ...
                                <aside className={'border-l border-l-accent flex flex-col justify-end pl-2'}>
                                    <Link href={'/tickets/code'} className={'border border-black rounded p-1'}>
                    ...
                    <div className={'w-full mb-2 rounded-lg border border-accent bg-white'}>
                        <header className={'w-full flex justify-center items-center bg-[#dfe7f4] rounded-lg rounded-b-none'}>
                            <h2 className={'p-2 text-right font-bold text-lg'}>Informations de contrôle</h2>
                        </header>
                        <div className={'p-3 flex flex-col items-center justify-center'}>
                            <p className={'flex items-center justify-center font-medium mb-1'}>
                                <CalendarCheck size={20} className={'text-success mr-2'} /> {createdAt.getDate()}/{createdAt.getMonth()+1}/{createdAt.getFullYear()} à {createdAt.getHours()}:{createdAt.getMinutes()}
                            </p>
                            <p className={'text-2xl font-bold text-[#202020]'}>202-658F6AE6A1193</p>
                        </div>
                    </div>
                    <div className={'w-full h-8 bg-white border border-black rounded-3xl'}>
                        <div className={`h-full bg-success rounded-3xl`} style={{
